...
#define check_bit(addr,x) 	((addr >> (x)) & 0x1)
	...
	for (int i=0; i<orig_ws; i++) {
		...
		if (g_color_cnt > 0) {
			...
			for (int j = 0; j < g_color_cnt; j++) {
				...
				if (((check_bit(vaddr,6) ^check_bit(vaddr,13))==0)&& 
                    ((check_bit(vaddr,14)^check_bit(vaddr,17))==0)&& 
                    ((check_bit(vaddr,15)^check_bit(vaddr,18))==0)&& 
                    ((check_bit(vaddr,16)^check_bit(vaddr,19))==0)) {
					if (g_debug)
